  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: "#f0f4ff", marginBottom: 6 }}>{t.reportsPage.title}</h1>
          <p style={{ color: "var(--text-secondary)", fontSize: 14 }}>{t.reportsPage.subtitle.replace("{n}", String(stores.length))}</p>
        </div>
        <button
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 28 }}>
        {[
          { icon: "🏪", label: t.reportsPage.statTotalProjects, value: stores.length, color: "#3b82f6" },
          ...
          { icon: "🚀", label: t.reportsPage.statOpened, value: stores.filter(s => s.status === "COMPLETED").length, color: "#f59e0b" },
        ].map((card) => (
          ...
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "#f0f4ff", marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 13, color: "var(--text-secondary)", marginLeft: 10, fontWeight: 400 }}>
              {t.reportsPage.regionSummary
                ...
                .replace("{p}", String(Math.round(regionStores.reduce((s: number, st: any) => s + st.progress, 0) / regionStores.length)))}
            </span>
          </h2>
                ...
                {(regionStores as any[]).map((store: any) => {
                  ...
                  return (
                    <tr key={store.id}>
                      ...
                      <td>{store.pm?.name || "—"}</td>
                      <td><span className={`badge ${STATUS_COLORS[store.status]}`}>{getStatusLabel(store.status, locale)}</span></td>
                      <td>
                      ...
                      <td style={{ fontSize: 12 }}>
                        {activePhase ? t.reportsPage.activePhaseFmt.replace("{n}", String(activePhase.phaseNumber)).replace("{name}", activePhase.name) : "—"}
                      </td>
