...
const TASKS_BY_PHASE: Record<number, string[]> = {
  ...
  3: [
    ...
    "Đàm phán điều khoản phụ lục: biển hiệu, sửa chữa, thoát hiểm",
  ...
  7: [
    "Lập danh sách thiết bị cần mua (POS, máy tính, camera, bàn ghế, kệ)",
  ...
  8: [
    "Xác định cơ cấu nhân sự (Store Manager, GDV, bảo vệ, vệ sinh)",
    "Đăng tuyển dụng (website nội bộ, TopCV, Facebook)",
    "Phỏng vấn và lựa chọn ứng viên",
    "Ký hợp đồng lao động",
    "Làm thủ tục onboarding (nhân sự, đồng phục, thẻ nhân viên)",
  ],
  9: [
  ...
  10: [
    ...
    "Chuẩn bị marketing khai trương (flyer, social media, SMS blast)",
  ],
  11: [
...
async function main() {
  ...
  const areaManager = await prisma.user.create({
    data: {
      name: "Trần Quản Lý Vùng",
      email: "manager@telecom.vn",
      password: hashedPassword,
      role: "AREA_MANAGER",
      region: "Hồ Chí Minh",
    },
  ...
  const storesData = [
    {
      ...
      status: "IN_PROGRESS",
      pmId: pm1.id,
      targetOpenDate: new Date("2025-09-01"),
      budget: 850000000,
      activePhase: 6, // đang ở giai đoạn xây dựng
    },
    {
      name: "Cửa Hàng Bình Thạnh - Xô Viết",
      code: "HCM-BT-002",
      address: "302 Xô Viết Nghệ Tĩnh, Phường 25, Bình Thạnh, TP.HCM",
      region: "Hồ Chí Minh",
      status: "IN_PROGRESS",
      pmId: pm1.id,
      targetOpenDate: new Date("2025-10-15"),
      budget: 650000000,
      activePhase: 3, // đang đàm phán
    },
    {
      name: "Cửa Hàng Hoàn Kiếm - Hàng Bài",
      code: "HN-HK-001",
      address: "56 Hàng Bài, Phường Trần Hưng Đạo, Hoàn Kiếm, Hà Nội",
      region: "Hà Nội",
      status: "PLANNING",
      pmId: pm2.id,
      targetOpenDate: new Date("2025-12-01"),
      budget: 920000000,
      activePhase: 1, // đang tìm kiếm
    },
  ];
  ...
  for (const storeData of storesData) {
    ...
    // Tạo 11 phases cho mỗi store
    for (const phaseTemplate of PHASES) {
      const phaseStatus =
        phaseTemplate.phaseNumber < activePhase
          ? "COMPLETED"
          : phaseTemplate.phaseNumber === activePhase
          ? "IN_PROGRESS"
          : "NOT_STARTED";
      ...
      for (let i = 0; i < taskTitles.length; i++) {
        const taskStatus =
          phaseStatus === "COMPLETED"
            ? "DONE"
            : phaseStatus === "IN_PROGRESS" && i < Math.floor(taskTitles.length / 2)
            ? "DONE"
            : phaseStatus === "IN_PROGRESS" && i === Math.floor(taskTitles.length / 2)
            ? "IN_PROGRESS"
            : "TODO";
        ...
        await prisma.task.create({
          data: {
            ...
            assigneeId:
              phaseTemplate.phaseNumber <= 2
                ? surveyor.id
                : phaseTemplate.phaseNumber <= 4
                ? pm1.id
                : store.pmId,
          },
        });
    ...
    if (storeData.activePhase >= 3) {
      await prisma.issue.create({
        data: {
          ...
          reporterId: pm1.id,
          resolution:
            storeData.activePhase > 4
              ? "Đã đàm phán thành công, đồng ý tăng 5% với điều kiện kéo dài thời hạn thuê thêm 1 năm"
              : null,
        },
      });
    }
    ...
    await prisma.activity.create({
      data: {
        ...
        entity: "Phase",
        entityId: store.id,
        details: `Cập nhật trạng thái giai đoạn ${activePhase} sang Đang thực hiện`,
      },
    });
  }
